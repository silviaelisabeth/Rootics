...
class IntroPage(QWizardPage):
    ...
    def initUI(self):
        ...
        self.o2_box.setFont(QFont(font_button, fs_intro)), self.ph_box.setFont(QFont(font_button, fs_intro)),
        self.h2s_box.setFont(QFont(font_button, fs_intro)), self.ep_box.setFont(QFont(font_button, fs_intro))
        self.combo_box.setFont(QFont(font_button, fs_intro))

        # path for measurement file (csv)
        self.load_button = QPushButton('Load meas. file', self)
        self.load_button.setFixedWidth(150), self.load_button.setFont(QFont(font_button, fs_intro))
        ...
        self.inputFileLineEdit.setFont(QFont(font_button, fs_intro))

        # directory to store files
        self.save_button = QPushButton('Storage path', self)
        self.save_button.setFixedWidth(150), self.save_button.setFont(QFont(font_button, fs_intro))
        ...
        self.inputSaveLineEdit.setFont(QFont(font_button, fs_intro))

        # saving options
        self.set_button = QPushButton('Settings', self)
        self.set_button.setFixedWidth(150), self.set_button.setFont(QFont(font_button, fs_intro))
...
class SettingWindow(QDialog):
    ...
    def initUI(self):
        ...
        self.meta_box.setChecked(True), self.meta_box.setFont(QFont(font, fs_font))
        self.rdata_box = QCheckBox('Raw data', self)
        self.rdata_box.setChecked(True), self.rdata_box.setFont(QFont(font, fs_font))
        self.fit_box = QCheckBox('Fit data', self)
        self.fit_box.setChecked(True), self.fit_box.setFont(QFont(font, fs_font))
        self.adj_box = QCheckBox('Adjusted data', self)
        self.adj_box.setChecked(True), self.adj_box.setFont(QFont(font, fs_font))
        self.pen_box = QCheckBox('Penetration depth', self)
        self.pen_box.setChecked(True), self.pen_box.setFont(QFont(font, fs_font))

        self.swiRaw_box = QCheckBox('Raw profile', self)
        self.swiRaw_box.setChecked(False), self.swiRaw_box.setFont(QFont(font, fs_font))
        self.swiF_box = QCheckBox('Adjusted profile', self)
        self.swiF_box.setChecked(False), self.swiF_box.setFont(QFont(font, fs_font))
        self.fitF_box = QCheckBox('Fit plot', self)
        self.fitF_box.setChecked(False), self.fitF_box.setFont(QFont(font, fs_font))
        self.penF_box = QCheckBox('Penetration depth', self)
        self.penF_box.setChecked(False), self.penF_box.setFont(QFont(font, fs_font))
...
def GUI_penetration_av(core, ls_core, dcore_pen, fig=None, ax=None, show=True):
    ...
    if core_select != 0:
        ...
        for en, s in enumerate(df.columns):
            if s in ls_remain:
                ax.plot(df[s].dropna().sort_index(), df[s].dropna().index, color=ls_col[en], lw=1.5, alpha=0.5,
                        label='sample-' + str(s))

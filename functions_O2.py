...
def plot_penetrationDepth(core, s, df_fit, O2_pen, unit, show=False):
    ...
    ax.set_title('Penetration depth for core ' + str(core) + ' sample-' + str(s))
    ...
    if core_select != 0:
        ...
        for en, s in enumerate(df.columns):
            if core_select in dobj_hid.keys():
                ...
                alpha_ = .6
            d = df[s].dropna().sort_index()
            line, = ax.plot(d.values, d.index, color=ls_col[en], lw=1.5, alpha=alpha_, pickradius=6,
                            label='sample-' + str(s))
            lines.append(line)
